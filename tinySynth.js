...
class TinySynth {
    static soundFont = null;    // 最终是填补了默认值的TinySynth.wave
    static instrument = [
...
/* 121-128 : SE */	"Guitar Fret Noise", "Breath Noise", "Seashore", "Bird Tweet", "Telephone Ring", "Helicopter", "Applause", "Gunshot",
     ...
     */
    static initSoundFont({ g = 0, w = "sine", t = 1, f = 0, v = 0.5, a = 0, h = 0.01, d = 0.01, s = 0, r = 0.05, p = 1, q = 1, k = 0 } = {}) {
        if (!TinySynth.soundFont) TinySynth.soundFont = {};
    ...
    constructor({ actx = new AudioContext() } = {}) {
        if (!TinySynth.soundFont) TinySynth.initSoundFont();
    ...
    addChannel(at) {
        const ch = {
            out: this.actx.createGain(),
            instrument: 0
        };
        Object.setPrototypeOf(ch, TinySynth);
        if (at === void 0) at = this.channel.length;
        this.channel.splice(at, 0, ch);
        return ch;
    }
    play({ id, f = 440, v = 100, t = 0, last = 9999 } = {}) {  // t表示声音发出的时间
    ...
    releaseNote(nt, t = this.actx.currentTime) {
        for (let i = 0; i < nt.gain.length; i++) {
            nt.gain[i].gain.cancelScheduledValues(t);
            // 包络的R
            nt.gain[i].gain.setTargetAtTime(0, t, nt.release[i]);
            nt.osc[i].stop(t + nt.release[i]);
